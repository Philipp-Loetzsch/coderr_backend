...
# class Order(models.Model):
#     offer = models.ForeignKey(Offer, related_name='orders', on_delete=models.SET_NULL, null=True)
#     offer_detail = models.ForeignKey(OfferDetail, related_name='orders', on_delete=models.SET_NULL, null=True)
#     buyer = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='orders_bought', on_delete=models.CASCADE)
#     seller = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='orders_sold', on_delete=models.CASCADE)
...
#         offer_title = self.offer.title if self.offer else "Gelöschtes Angebot"
#         detail_title = self.offer_detail.title if self.offer_detail else "Gelöschtes Detail"
#         return f"Bestellung #{self.id}: '{detail_title}' von '{offer_title}'"
...
# Importiere OfferDetail aus der offers App
# Passe den Importpfad ggf. an deine Projektstruktur an
from offers_app.models import OfferDetail

class Order(models.Model):
    """Modell für eine Bestellung eines Angebots-Details."""
    ...
    customer = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE, # Was passiert, wenn der Kunde gelöscht wird?
        related_name='orders_as_customer',
        verbose_name=_("Customer")
    )
    offer_detail = models.ForeignKey(
        OfferDetail,
        on_delete=models.CASCADE, # Was passiert, wenn das OfferDetail gelöscht wird? Ggf. PROTECT?
        related_name='orders',
    ...
    # Der Anbieter kann über self.offer_detail.offer.user erreicht werden
